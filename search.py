...
def dfsExplore(state,exploredState,exploredPath,problem):
    isGoal=False
    if problem.isGoalState(state): return exploredPath, True
    for successor in problem.getSuccessors(state):
        nextState, action, cost = successor
        if nextState not in exploredState:
            exploredState.append(nextState)
            exploredPath.push(action)
            exploredPath,isGoal = dfsExplore(nextState,exploredState,exploredPath,problem)
            if isGoal: return exploredPath, isGoal
            exploredPath.pop()
    return exploredPath,isGoal
...
def uniformCostSearch(problem):
    ...
    while not Q.isEmpty():
        ...
        for successor in problem.getSuccessors(node):
            ...
            else: childDist = tempCost[explored.index(state)]
            if pathCost + cost < childDist :
                childDist = pathCost + cost
                Q.update([state,path+[action]],childDist)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not Q.isEmpty():
        ...
        for successor in problem.getSuccessors(node):
            ...
            if pathCost + cost < childDist:
                childDist = pathCost + cost
                Q.update([state, path + [action]], childDist+heuristic(state,problem))
                tempCost[explored.index(state)] = childDist
    return []
